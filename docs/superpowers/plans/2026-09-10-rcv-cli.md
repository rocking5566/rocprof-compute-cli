...
- Task 8: `hotspot` command wiring (`--top`, `--by`, `--sort`, `--json`) — the query layer and formatter already exist, so this is argument parsing plus a table renderer
- Task 9: `asm` command (`--range A-B`, `--around N`, `--context N`) with per-line stall-reason columns
- Task 10: `occupancy` command (`--bins`, `--se`)
- Task 11: End-to-end smoke test asserting non-zero hidden latency and recording wall time and peak RSS
- Task 12: ATT decoder input path, validated to produce a digest matching the JSON path
