...
- `rcv_core` and `rcv-cli` link **no Qt at all**. Never add any `Qt6::` target to them. A CLI-only build works on a machine with no Qt installed.
...
- **Qt is needed only for `RCV_BUILD_GUI=ON`.** CLI and test builds need no Qt and no `-DQT_VERSION_MINOR` flag. If you do configure a GUI build, the installed Qt is 6.4.2 and `CMakeLists.txt` defaults `QT_VERSION_MINOR` to 8, so pass `-DQT_VERSION_MINOR=4` in that case only.
...
| 40-48 | `PosToClock`, `ClocksPerPixel` | **guard** — both use `MainWindow::getScaling()` and every caller is GUI-side |
...
./build-tests/bin/rcv_core_link_test
```
...
```bash
ldd ./build-tests/bin/rcv_core_link_test | grep -i widgets; echo "exit=$?"
```
...
cmake --build build-tests --target trace_loader_test --parallel && \
  ./build-tests/bin/trace_loader_test
```
...
- [ ] **Step 0: Clamp the hidden components in LineDigest**

`digest.h` from Task 3 sums the raw components. Replace `hidden()` and add clamped
per-component accessors, mirroring `HorizontalHotspot::hiddenTotal()` (`hotspot.hpp:46-57`).
Keep the raw fields as they are: the digest stores raw and clamps on read, so the raw numbers
...
Add `#include <algorithm>` to `digest.h`. `queries.cpp` accumulates `l.hidden()` into
`HotspotRow::hidden`, so it picks the clamped value up automatically — no change needed there.
...
TEST(HiddenLatency, RawComponentsDoOverflowSomewhere)
{
    ...
    int overflow = 0;
    for (const auto& l : d.lines)
        if (l.hidden_idle > l.idle || l.hidden_stall > l.stall || l.hidden_issue > l.issue())
            ++overflow;
...
cmake --build build-tests --target hidden_latency_test --parallel && \
  ./build-tests/bin/hidden_latency_test
```
...
cmake --build build-tests --target occupancy_test --parallel && \
  ./build-tests/bin/occupancy_test
```
...
```bash
cmake --build build-tests --target queries_test --parallel && ./build-tests/bin/queries_test
```
...
```bash
cmake --build build-tests --target format_test --parallel && ./build-tests/bin/format_test
```
...
cmake -S tests -B build-tests -DCMAKE_BUILD_TYPE=Release \
  -DTRACE_DECODER_ROOT=$HOME/rocm7.15.0a20260712 && \
cmake --build build-tests --target smoke_test --parallel && ./build-tests/bin/smoke_test
```
...
TEST(AttPath, IdleDivergesOnlyWithinDecoderVersionSkew)
{
    ...
    std::cerr << "[ INFO     ] idle: " << differing << " lines differ (" << line_share
              << "%), aggregate delta " << delta << " of " << total << " (" << cycle_share
              << "%), first at line " << first_diff << "\n";
...
cmake --build build-tests --target att_path_test --parallel && ./build-tests/bin/att_path_test
```
...
Expected: `[  PASSED  ] 3 tests.` The five direct per-instruction fields assert exact equality
on purpose — if any of those differ, report the first differing line index rather than loosening
the test. `idle` is the one measured exception and has its own bounded test; see the comment
above it for the evidence.

Add `#include <cstdlib>` and `#include <iostream>` to the test file for `std::llabs` and the
info line.
...
cmake --build build-tests --target synthetic_test --parallel && \
  ./build-tests/bin/synthetic_test
```
